...
export namespace LS
{
	...
	uint32_t GetShaderDataByteSize(SHADER_DATA_TYPE type)
	{
		...
		{
		case SDT::NONE:
			throw std::runtime_error("Cannot build layout stride for LSShaderInputLayout with a unspecified data type.\n");
			break;
	 ...
	 */
	uint32_t GetShaderDataTypeCount(LS::SHADER_DATA_TYPE type)
	{
		using SDT = LS::SHADER_DATA_TYPE;
		switch (type)
		{
	...
	inline size_t HashShaderElement(LS::ShaderElement const& elem) noexcept
	{
		std::size_t h1 = LS::Utils::HashEnum(elem.ShaderData);
		std::size_t h2 = std::hash<uint32_t>{}(elem.OffsetAligned);
		std::size_t h3 = LS::Utils::HashEnum(elem.InputClass);
	...
	/**
	* @brief An object that cotains information regarding how a vertex buffer's data is laid out.
	*
	* Without a Vertex Description object, the device won't know how to interpret the data
	* in a vertex buffer. Vertex Buffers can contain information regarding position, color, normal, texture coordinates, and more.
	*/
	class LSShaderInputLayout
	{
	public:
		LSShaderInputLayout() = default;
		LSShaderInputLayout(std::span<LS::ShaderElement> elems)
		{
		...
		const std::vector<LS::ShaderElement>& GetInputLayout() const
		{
		...
		void SetInputLayout(std::span<LS::ShaderElement> elems)
		{
		...
		*/
		void AddElement(const LS::ShaderElement& desc)
		{
			m_inputLayout.emplace_back(desc);
		}

		void AddElement(LS::SHADER_DATA_TYPE dataType, std::string_view semanticName,
			uint32_t offsetAligned = 0,
			LS::INPUT_CLASS inputClass = LS::INPUT_CLASS::VERTEX,
			uint32_t inputSlot = 0,
			uint32_t instanceStepRate = 0)
		{
			LS::ShaderElement elem{ .ShaderData = dataType,
		...
		bool operator==(const LS::LSShaderInputLayout& rhs) const
		{
	...
	protected:
		std::vector<LS::ShaderElement> m_inputLayout;
		uint32_t m_stride = 0;
	};
}